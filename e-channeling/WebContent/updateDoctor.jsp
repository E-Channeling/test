...
<html lang="en">
  ...
  <body style="background-image: url(images/hero.jpg);">
    ...
    <%
		...
		ArrayList<Doctor> doctortList = new ArrayList<Doctor>();
		doctortList = doctorService.getDoctorByID(id);
    %>
        <form method="post" target="_self" class="form"
					action="UpdateDoctorServlet">
					<h1>Profile Update</h1>
					<div class="form-group">
					<label>First Name</label> 
					<input type="text" class="form-control"
						value="<%=doctortList.get(0).getFirstName()%>" required autofocus name="fName">
					<label>Last Name</label> 
					<input type="text" class="form-control"
						value="<%=doctortList.get(0).getLastName()%>" required autofocus name="lName"> 
					<label>Phone Number</label> 
					<input type="number" class="form-control" placeholder="<%=doctortList.get(0).getContact()%>"
						required autofocus name="phoneNo"> 
					<label>Address</label>
					<input type="text" class="form-control" value="<%=doctortList.get(0).getAddress()%>"
						required autofocus name="address"> 
					<label>Email address</label> 
					<input type="email" class="form-control"
						value="<%=doctortList.get(0).getEmail()%>" required autofocus name="email"> </br> 
